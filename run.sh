...
# Install required Python packages without specific versions
pip install -r requirements.txt

# Check if the OpenVPN script exists
if [ ! -f "./ubuntu-openvpn-install.sh" ]; then
    echo "Creating OpenVPN installer script..."
    cat > ./ubuntu-openvpn-install.sh << 'ENDOFSCRIPT'
#!/bin/bash
...
# Make sure the OpenVPN script is executable
chmod +x ./ubuntu-openvpn-install.sh

# Create a dummy openvpn status file for testing if not in prod environment
if [ ! -d "/etc/openvpn" ] && [ ! -f "/etc/openvpn/server.conf" ]; then
    echo "Creating dummy OpenVPN status for development environment"
    mkdir -p /tmp/openvpn/easy-rsa/pki
    echo "V CN=client1" > /tmp/openvpn/easy-rsa/pki/index.txt
fi
